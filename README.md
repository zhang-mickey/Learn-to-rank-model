...
## 负采样
负采样是指从非相关项中随机选择样本，与正样本（即用户实际交互过的项）一起用于训练模型。负采样的目标是帮助模型更好地学习区分相关和非相关项，从而提高推荐的准确性。
...
我们在每个批次训练过程中，除了使用当前批次的负样本外，还会从之前批次的嵌入向量中随机选择一部分作为额外的负样本。这些额外的负样本与当前批次的负样本一起，用于计算损失函数并更新模型参数。
